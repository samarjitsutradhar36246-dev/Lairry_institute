...
export default function QuestionCSVActions({
  ...
  const processRows = (rows) => {
    ...
    const processedRows = rows.map((row, index) => normalizeRow(row, index));
  ...
  const normalizeRow = (row) => {
    ...
    const positiveMark = Math.abs(Number(row.positive_mark || 1)); // optional: ensure positive mark stays positive
    const negativeMark = -Math.abs(Number(row.negative_mark || 0)); // auto-convert to negative
    ...
    if (isNaN(expectedTime))
      errors.expected_time_for_each_question = "Invalid number";
  ...
  const exportErrorsCSV = () => {
    const failed = previewRows.filter((r) => Object.keys(r.errors).length > 0);
          ...
          <Box sx={{ maxHeight: 500, overflow: "auto" }}>
            <Box mb={2}>
              <Alert
                ...
                {
                  previewRows.filter((r) => Object.keys(r.errors).length > 0)
                    .length
                }{" "}
                rows contain errors.
              </Alert>
                  ...
                  <TableRow
                    ...
                    <TableCell
                      onDoubleClick={() =>
                        setEditingCell({
                          rowIndex: i,
                          field: "expected_time_for_each_question",
                        })
                      }>
                      {editingCell.rowIndex === i &&
                      editingCell.field ===
